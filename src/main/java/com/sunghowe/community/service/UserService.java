...
public class UserService implements CommunityConstant {
    ...
    public Map<String, Object> register(User user) {
        ...
        user.setHeaderUrl(String.format("http://images.newcoder.com/head/%dt.png",
    ...
    public Map<String, Object> login(String username, String password, int expiredSeconds){
        ...
        if (StringUtils.isBlank(username)) {
            map.put("usernameMsg", "账号不能为空哦^_^");
            return map;
        }
        if (StringUtils.isBlank(password)) {
            map.put("usernameMsg", "密码不能为空哦^_^");
            return map;
        }
        ...
        if (user == null) {
            map.put("usernameMsg", "账号不存在啊-_-");
            return map;
        }
        //验证状态
        if (user.getStatus() == 0){
            map.put("usernameMsg", "账号未激活呀-_-");
            return map;
        }
        //验证密码
        password = CommunityUtil.md5(password + user.getSalt());
        if (!user.getPassword().equals(password)) {
            map.put("passwordMsg", "密码不正确呢-_-");
            return map;
        }
